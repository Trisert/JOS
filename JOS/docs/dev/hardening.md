...
| 1.4 | No dynamic allocation after init (`-Werror=implicit-function-declaration` already catches much); ban `malloc/free` inside tasks | [NASA-PoT] #5, [JPL-182] | High | Low (policy) |
...
| 5.2 | **SEU mitigation**: periodically re-write critical RAM structures (state, config) from FRAM; use the SRAM2 parity NMI to detect corruption | [NASA-STD-8739.8] (fault tolerance) | Med | Med | **DONE (W2-5)** — `App/obsw/scrub.c`; boot repair + 5 s scrub pass + write-through; doc `docs/dev/scrub.md` |
| 5.3 | **FRAM**: already non-volatile and radiation-tolerant (FeRAM) — good choice; add a CRC per FRAM record | [ECSS-E-ST-40C] | Low | Low | **DONE (W2-5)** — every golden record carries a CRC-32; corrupt backup is rejected, never restored |
