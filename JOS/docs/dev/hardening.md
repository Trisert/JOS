...
64 KB SRAM2 with parity, 4× FM24VN10-G FRAM on I²C1 (PB8/PB9) = 512 KB total (4 × 128 KB)).
...
| 2.4 | **Watchdog** — **DONE**: software monitor in `App/obsw/watchdog.c` (500 ms tick, flags silence >3× period); `watchdog_register_task()` is called from **7** task creators (`main.c:253` defaultTask, `comms.c:215/260`, `state_machine.c:426`, `clear.c:159`, `cloud.c:148`, `aocs.c:43`) and `watchdog_alive_self()` runs in every task loop. Hardware IWDG is **also active** (~31 s, `Core/Src/hw_watchdog.c` configures it, `main.c:115` inits, `watchdog.c`/main kick it). The monitor **never suspends the LastStates pool-mutex holder** (`watchdog_suspend_allowed()` over `xQueueGetMutexHolder()`): the escalation is deferred, counted (`watchdog_holder_deferrals()`) and retried next scan. Every scan samples `uxTaskGetStackHighWaterMark()` per task for `watchdog_task_stack_hwm()` telemetry | [NASA-STD-8739.8] (watchdog/monitoring), [ECSS-E-ST-40C] | High | Med |
...
| 2.6 | **Malloc-failed hook** — **DONE**: `vApplicationMallocFailedHook` (`Core/Src/freertos.c`) is no longer silent: `fault_log_malloc_failed()` (`Core/Src/faults.c`) records free/min-ever-free heap in the LastStates pool (`TRIGGER_MALLOC_FAILED`) and reboots, mirroring the stack-overflow hook (2.3) | [NASA-STD-8739.8] (fault containment) | High | Low |
| 2.7 | **Flash write under `state_mutex`** — **BOUND DOCUMENTED** (`App/obsw/state_machine.c:try_transition()`): the LastStates log stays inside the critical section deliberately (log-then-commit order is load-bearing), with a documented worst-case hold (nominal ~= 160 ms: bounded 80 ms own program + one bounded pool-lock wait; lock order `state_mutex` -> pool mutex, prio-inherit). The bound terminates because the pool-mutex holder is never suspended (see 2.4) | [NASA-STD-8739.8] | High | Low |
...
| 5.3 | **FRAM**: already non-volatile and radiation-tolerant (FeRAM) — good choice; add a CRC per FRAM record | [ECSS-E-ST-40C] | Low | Low | **PARTIAL** — the SEU golden-copy path stores CRC-32-protected records (`Core/Src/seu_mitigation.c:467-480,524-540`); the cyclic science buffer still performs bounded raw I2C transfers with no per-record CRC (`App/memory/memory.c:282-315`). |
