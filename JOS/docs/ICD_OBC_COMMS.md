...
| GPIO_INT | PB0 / **EXTI0** | 15 | DIO1/IRQ | IN, EXTI rising | NVIC prio 5 (logical; HW register 0x50 = 80 = ceiling), enabled |
...
- Priorità **5** (logica) = `configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY`:
  `HAL_NVIC_SetPriority` codifica da sé (gruppo 4 già fissato da `HAL_Init()`,
  shift << 4) quindi il registro vale 0x50 = 80 = syscall ceiling, e la ISR può
  chiamare API FreeRTOS ISR-safe (`osThreadFlagsSet`). Mai passare 80 a questa
  API: verrebbe mascherato a priorità 0 (massima) con assert al primo interrupt.
