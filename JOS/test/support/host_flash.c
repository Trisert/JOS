 ...
 *   - programming requires the Flash to be unlocked.
 *
...
{
    size_t chip = (size_t)(dev_addr - 0x50U);

    if (dev_addr < 0x50U || dev_addr > 0x53U) {
        return -1;
    }
    if (((size_t)mem_addr + size) > (16U * 1024U)) {
        return -1;                            /* would cross a chip boundary */
    }

    *out = (chip * 16U * 1024U) + mem_addr;
    return 0;
}
