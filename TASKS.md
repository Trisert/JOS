...
| 12 | done    | **Unblock Ceedling CI** — root cause found: `test_lora_rx_task_loop_delays_at_the_registered_period` spins forever because the RX task is event-driven (no `osDelay` since PR #47); every Ceedling run since 2026-08-14 died at the 6 h timeout | PR [#50](https://github.com/Trisert/JOS/pull/50) | MERGED 2026-08-22. All 4 checks green on the PR (Ceedling 28 s vs 6 h hang). Local verify: 142/142 host tests on aarch64. Post-merge main run: see Actions. |
| 13 | todo | **Local branch cleanup** — 11 merged branches to delete (`archive/pr17head` tag first); blocked from Discord by approval-prompt delivery | — | Commands ready; run them in a local session or terminal. Also optional: delete merged remote branches. |
