...
| 32 | done | **Subagent reviews (criterio 3)**: #70 APPROVE, #63 APPROVE (rework 4711d61: CSELR, pin-map, DMA reale IRQ; Kilo pending), #72 APPROVE, #68 APPROVE, #69 REQUEST-CHANGES→fixato `cr=8` (`af29496`), #73 APPROVE (Kilo fail→rework in corso). | — | #63 mergiabile (Kilo pending). |
| 33 | done | **FRAM full-size 128KB/chip (512KB)** → PR #73 `fix/fram-size`: A16 in slave byte, decode chip=addr>>17, fakes+2 test, build ok, 146/146. | [#73](https://github.com/Trisert/JOS/pull/73) | Subagent review: APPROVE (nessun aliasing, bound ok). Resta: conferma strapping A2/A1 (Nicola). |
