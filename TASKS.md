...
| 16 | done    | **T33 — DMA SPI1 (SPF §3.6.4.2)**: configurare DMA per SPI1 (SX1268) con DMA1_Channel2/Channel3 + DMAMUX request 11/12. In worktree `feat/dma-spi1-sx1268`. Compilato, PR da creare. | — | [Worktree:](file:///root/JOS-t33a-dma-spi1) `feat/dma-spi1-sx1268`. DMA configurato ma IRQ disabilitati (polling), polling OK compilazione. **PR #63 chiusa (non mergeata)** — DMA IRQ disabilitati viola "no polling" SPF §3.6.4.2. Fix: IRQ Channel2/3 abilitati + callback.** 67 KB.** |
...
| 28 | done | **Fix LoRa CR 4/9→4/8 (SPF Tab. 3.28)** — worktree `/root/JOS-lora-cr`, branch `fix/lora-cr-4-8`, `cr=5`→`4` in `radiolib_driver.cpp:60`, `make release` exit 0. | [#69](https://github.com/Trisert/JOS/pull/69) | CI VERDE (firmware, ceedling, static-analysis). Non mergiata (attende via). |
| 29 | doing | **T35 TIM6 (SPF Tab. 3.18)** — subagent: `fix/t35-tim6-timebase`, TIM6 HAL timebase + SysTick solo FreeRTOS, `make release` ok, ceedling 144/144. | [#70](https://github.com/Trisert/JOS/pull/70) | static-analysis fail su 1 style (constParameterPointer) → fixato con inline suppress (firma deve matchare HAL weak), pushato `447c37d`, CI re-run. |
| 30 | doing | **FRAM→SPI2+DMA (SPF §3.6.4.2)** — subagent: `fix/fram-spi2`, memory.c su SPI2, CS definiti, DMA Ch4/5, `make release` ok, 144/144. | [#71](https://github.com/Trisert/JOS/pull/71) | PR aperta, CI in corso. Flag: PA4 conteso tra FRAM CS e radio CS_TTC TODO — decisione Nicola. |
| 31 | doing | **TEMP 1-wire PB2 + DEPLOY/LoRa_NRST mux PB1 (SPF §3.7.5.3.1)** — subagent in corso su `/root/JOS-pins`. | — | Netlist: TMP1827 x4 su PB2, LoRa/DEPLOY off-board, PB1 libero. |
